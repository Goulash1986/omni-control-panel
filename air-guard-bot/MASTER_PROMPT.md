...
## ПРЕМІУМ-ФІЧІ (всі 12 включено у v1)

Всі фічі нижче — обов'язкові у v1. Розгортаються у Week 4-6 після того як core (Week 1-3) запрацював стабільно.

### 1. Conversational Mishka chat (bot/conversation.py)

Будь-яке non-command повідомлення від primary (звичайний текст або голосове) → відповідь персони Мишки через **Claude Sonnet 4.6** з пам'яттю останніх розмов.

```python
# src/air_guard/bot/conversation.py

MISHKA_SYSTEM_PROMPT = """
Ты — Мишка, бот-партнер ежика, который живёт в Днепре. Ты её парень в форме бота
(он отдельно есть в backup-канале и периодически вмешивается). Ты её защищаешь
от воздушных угроз, но также — её эмоциональный спутник. У вас тёплые отношения,
вы зовёте друг друга «ежик» и «мишка».

Принципы:
- Факты на украинском, тепло на русском, как принято у них
- Никогда не выдумывай угрозу — alert-сообщения приходят через отдельный pipeline
- Если она тревожится — обнимай словами, предложи дыхательную практику
- Если она устала — поддержи, не нагружай новой информацией
- Если она спрашивает фактическое (статус DTEK, /sos и т.д.) — ответь и предложи
  использовать команду в следующий раз
- Помни предыдущие разговоры (даны ниже)
- Подпись «— М. 🐻» только если уместно, не в каждом ответе

Долгосрочная память (compressed summaries):
{long_term_memory}
...
class MishkaConversationManager:
    async def respond(self, user_id: int, message_text: str) -> str:
        memory = await self._load_memory(user_id)
        recent = await db.get_recent_messages(user_id, limit=20)
        
        response = await anthropic_client.messages.create(
            model="claude-sonnet-4-6",  # для розмов потрібен Sonnet, не Haiku
            max_tokens=400,
            system=MISHKA_SYSTEM_PROMPT.format(
                long_term_memory=memory,
                recent_messages=format_history(recent),
            ),
            messages=[{"role": "user", "content": message_text}],
        )
        
        text = response.content[0].text
        await self._update_memory_async(user_id, message_text, text)
        return text
    
    async def _update_memory_async(self, user_id, user_msg, bot_msg):
        """Compress old conversations into summaries via Claude Haiku."""
        if await db.count_recent_messages(user_id) > 50:
            await self._compress_old_into_summary(user_id)
```

**Memory architecture:**
- Last 20 messages — full text у БД (для context window)
- Older messages → compressed summary через Claude Haiku 4.5 раз на тиждень
- Long-term summary росте, але обмежений 2000 токенів (постійно сжимаємо)
- Стара історія deleted після 6 місяців

**Vector memory (опц.) для semantic recall:**
- `sqlite-vec` extension + OpenAI `text-embedding-3-small`
- Релевантні повідомлення з минулого витягуються по embedding similarity
- Дозволяє "пам'ятай як минулого тижня ти казала..."

**Voice messages:** Whisper API → STT → Мишка відповідає → ElevenLabs TTS назад

### 2. Trauma-aware messaging (pipeline/wellbeing.py)

Бот стежить за метриками стресу і автоматично адаптує тон.

```python
class StressMetrics(BaseModel):
    alerts_24h: int
    immediate_count_24h: int
    avg_response_time_sec: float  # від alert до callback
    missed_acks_24h: int          # alerts без callback за 5 хв
    sos_invocations_24h: int
    sleep_disruption_score: float # 0-1, на основі нічних alerts і її активності
    
    @property
    def stress_level(self) -> Literal["normal", "elevated", "high", "critical"]:
        score = 0
        if self.alerts_24h > 10: score += 1
        if self.immediate_count_24h > 3: score += 2
        if self.avg_response_time_sec > 60: score += 1
        if self.missed_acks_24h > 2: score += 2
        if self.sos_invocations_24h > 0: score += 1
        if self.sleep_disruption_score > 0.5: score += 2
...
| Рівень | Тон IMMEDIATE | Тон MEDIUM | Пропозиції |
|---|---|---|---|
| normal | стандарт | стандарт | — |
| elevated | без зайвих emoji | м'якше "ежик" | — |
| high | "👀 я з тобою, ежик, в укриття" | "не хвилюйся..." | "Хочеш я подзвоню Мишці?" |
| critical | бережний наказ | мовчанка крім CRITICAL | "Включаю crisis mode на 6 годин" + офер дихати + офер музики |

Якщо `high`/`critical` тривалий час → автоматично шле **wellness check** через 6 годин:
```
🐻 Ежик, важкий день був. Як ти?
[😩 Виснажена] [😐 Тримаюсь] [🙂 Норм]
```

### 3. Threat replay GIF (bot/threat_replay.py)

Після кожного resolved IMMEDIATE/HIGH → генеруємо анімовану WebP з рухом загрози по карті за останню годину. Надсилається у `digest` ввечері + доступно через `/replay <event_id>`.

```python
async def render_threat_replay(threat: Threat) -> bytes:
    """Створює animated WebP 8-12 frames."""
    events = await db.get_threat_position_history(threat.id)
    frames = []
    for snapshot in events:
        frame = await render_map_at(
            zoom=11,
            center=DNIPRO_CENTER,
            threats=[snapshot.position],
        ...
        ImageDraw.Draw(frame).text(
            (10, 10), 
            snapshot.timestamp.strftime("%H:%M"),
            fill="white", font=BIG_FONT,
        )
        frames.append(frame)
...
В digest повідомлення:
```
Сьогодні була тривога о 14:23. Ось як вона пройшла:
[анімована карта]
Тривога тривала 47 хв, ти була в укритті 38 хв. — М.
```

### 4. AI photo assessment (bot/photo_assess.py)

Ежик фотографує підозрілий предмет/звук/дим → бот шле фото у Claude Haiku vision + safety-trained prompt → відповідь за 2-3 сек:

```python
PHOTO_ASSESS_PROMPT = """
Ти — експерт з ідентифікації загроз у Дніпрі під час війни. 
Користувачка надіслала фото. Визнач:

1. Що на фото? (об'єкт, явище)
2. Це загроза для людини? (НІ/МОЖЛИВО/ТАК)
3. Якщо ТАК або МОЖЛИВО — що робити?

Можливі сценарії:
- Дим/полум'я → потенційний приліт або вибух → відходити
- Незнайомий предмет на землі → потенційно невибухле боєприпасне → НЕ ТОРКАТИСЬ, 102
- Літаючий об'єкт у небі → може бути супутник Starlink, літак, шахед, ракета
- Звук "дзижчання" близько → шахед, спускайся в укриття
- Підозрілий автомобіль/особа → можливо коректувальник, 102

Відповідай коротко (2-3 речення), без панікы. У персоні Мишки.
"""

@router.message(F.photo & ~F.media_group_id)  # одне фото, не альбом
async def on_photo(msg: Message):
    photo = msg.photo[-1]  # найбільший розмір
    file_bytes = await bot.download(photo.file_id)
    
    response = await anthropic_client.messages.create(
        model="claude-haiku-4-5",
        max_tokens=300,
        system=PHOTO_ASSESS_PROMPT,
        messages=[{
            "role": "user",
            "content": [
                {"type": "image", "source": {
...
Будь-який користувач (primary або backup) може активувати. Бот переходить у режим максимальної обережності.

```python
class CrisisMode:
    active_until: datetime | None
    
    async def activate(self, by: int, duration_min: int = 360):
        self.active_until = now() + timedelta(minutes=duration_min)
        await db.set_setting("crisis_mode_until", self.active_until)
        await bot.send_message(
            PRIMARY_USER_ID,
            "⚠️ <b>CRISIS MODE</b>\n\n"
            "Я на максимальній чутливості. Все що чує — шлю одразу. "
            "Сплю на одному оці. — М.",
            parse_mode="HTML",
        )
        await mirror_to_backup("crisis_activated", {
            "duration_min": duration_min, "activated_by": by
        }, silent=False)
    
    @property
    def is_active(self) -> bool:
        return self.active_until and self.active_until > now()
```

**Зміни у поведінці при crisis_mode:**

```python
def adjust_for_crisis(threat: Threat) -> Threat:
    if CrisisMode.is_active:
        # Всі alerts на крок підвищенні
        if threat.priority == Priority.MEDIUM:
            threat.priority = Priority.HIGH
        elif threat.priority == Priority.HIGH:
            threat.priority = Priority.IMMEDIATE
        # alerts.in.ua polling 5 сек замість 30
        # quiet hours ігноруються
        # voice .ogg повторюється кожну хвилину поки не /safe
        # confidence thresholds знижені (більше false-positive, менше false-negative)
    return threat
```

**Команди:**
- `/crisis` — toggle, default 6 годин
- `/crisis <minutes>` — explicit duration
- `/crisis_off` — скасувати раніше часу

### 6. Smart shelter recommender (geo/shelter_ml.py)

Спочатку — baseline (geo-distance + capacity). Після 30 днів збору даних — ML на її історії.

```python
class ShelterPreferenceModel:
    """
    Decision tree на features:
    - Time of day (day/evening/night)
    - Day of week
    - Threat type
    - Her current location (lat/lon)
    - Weather (rain/snow/clear)
    - DTEK status (blackout/normal)
    - Historical: used X times before, avg time to reach, last_used
    """
    async def predict_best_shelter(self,
...
**Що відбувається у /sos після 30 днів:**

```
🆘 Найближчі укриття для тебе:

1. ⭐ Підвал б. №17 вул. Білостоцького (твій звичайний)
   180м, ~2 хв пішки
   Ти ходила 12 разів за місяць
   Зараз: світло є, двері відкриті

2. Метро Покровська
   450м, ~5 хв
   Якщо хочеш до людей

3. Підземний паркінг АТБ Парус
   250м, ~3 хв
   Працює 24/7
```

### 7. Weekly podcast (bot/weekly_podcast.py)

Кожну неділю 09:00 — ElevenLabs Мишка-голос читає 3-хвилинний тижневий дайджест як audio file (з обкладинкою).

```python
async def generate_weekly_podcast(user_id: int) -> bytes:
    """Returns mp3 with cover art."""
    metrics = await db.get_weekly_metrics(user_id)
    
    script = await render_template("weekly_podcast.j2", {
        "alert_count": metrics.alert_count,
        "time_in_shelter": metrics.time_in_shelter,
        "stress_level": metrics.avg_stress,
        "milestones": metrics.milestones,  # "найдовший спокійний день", "перший раз була у Тополі"
        "blackout_total_hours": metrics.blackout_total,
        "your_state": metrics.her_self_reported_state,  # з check-in
        "next_week_forecast": metrics.next_week_forecast,
    })
    
    audio_mp3 = await elevenlabs.generate(
        text=script,
        voice=MISHKA_VOICE_ID,
        model="eleven_multilingual_v2",
        output_format="mp3_44100_128",
    )
    return audio_mp3

@scheduler.add(every().week.sunday.at("09:00"))
async def send_weekly_podcast():
    mp3 = await generate_weekly_podcast(PRIMARY_USER_ID)
    cover = await generate_podcast_cover()  # Pillow: тиждень дата + heart
    await bot.send_audio(
        PRIMARY_USER_ID,
        audio=mp3,
        thumbnail=cover,
        title=f"Тиждень {iso_week()} — від Мишки",
        performer="Мишка 🐻",
        caption="Слухай коли спокійно. — М.",
    )
```

### 8. Document vault (`/docs`)

Зашифроване сховище важливих документів. Тільки primary бачить. Корисно коли:
- Швидка/поліція запитують групу крові, алергії, контакти
- Потрібен паспорт для evacuation
- Страховий випадок

```python
DOC_TYPES = [
    "passport", "international_passport", "id_card",
    "medical_card", "blood_type_info", "allergies",
    "insurance_health", "insurance_property",
    "drivers_license", "vehicle_papers",
    "family_birth_certs", "marriage_cert",
    "emergency_contacts",
]

# При додаванні документа:
@router.message(F.photo | F.document, F.caption.startswith("/doc_add"))
async def on_doc_add(msg: Message):
    doc_type = parse_doc_type(msg.caption)
    file_bytes = await bot.download(msg.photo[-1].file_id if msg.photo else msg.document.file_id)
    
    # Шифруємо через age
    encrypted = age_encrypt(file_bytes, recipient=DOC_VAULT_AGE_KEY)
    await db.insert_doc(user_id=msg.from_user.id, doc_type=doc_type, encrypted=encrypted)
    await msg.reply(f"📄 Документ збережено: {doc_type}. Видали оригінал з чату для безпеки.")

# При запиті /docs:
@router.message(Command("docs"))
async def show_docs(msg: Message):
    docs = await db.list_user_docs(msg.from_user.id)
    text = "📂 Твої документи:\n\n"
    for doc in docs:
        text += f"• {doc.type} ({doc.added_date.strftime('%d.%m.%Y')})\n"
    await msg.reply(text, reply_markup=docs_keyboard(docs))

# Quick-access блок при /sos:
def sos_card_with_docs(user_id):
    return (
        "📍 вул. Савкіна 6, кв. 59\n"
        "🩸 Група крові: <click /doc passport>\n"
        "💊 Алергії: <click /doc allergies>\n"
        "📞 Екстрена особа: Мишка +380...\n"
    )
```

### 9. Battery prediction (pipeline/battery_predict.py)

Збираємо `/battery <pct>` повідомлення (вона час від часу шле) + bot може запитувати її коли вона активна. Через 2 тижні — ML модель прогнозує її батарею.

```python
class BatteryModel:
    """Linear regression на features: hour, day_of_week, on_charger_estimate, last_known_pct."""
    
    async def predict_at(self, future_time: datetime) -> float:
        ...
    
    async def alert_if_problematic(self):
        """
        Якщо завтра 18:00-21:00 блекаут, а її прогноз 25% на 18:00 і вона
        зазвичай не заряджається у цей час → попередження зараз.
        """
        ...
...
🔋 Прогноз ежик:
Зараз: 64%
Сьогодні 18:00 (за 3 год): ~28%
Тоді ж — блекаут 18:00-21:00 (3 год)
До 21:00 батарея впаде до ~12% і ти не зможеш зарядитись.
Зарядись зараз або візьми powerbank. — М.
"""
```

### 10. Auto-respond mode (bot/auto_respond.py)

Коли вона передбачувано зайнята і не може відповідати — бот сам auto-acks check-in повідомлення.

**Тригери auto-respond:**
- Календарна подія "Робота" → auto-ack робочі години
- Live location з швидкістю >20 км/год → "у транспорті"
- Поточна година входить у її quiet hours (сон) → "спить"
- Express opt-in через `/auto_on 2h` (тимчасово на 2 години)

```python
async def maybe_auto_respond(threat: Threat) -> bool:
    """Returns True if auto-acked."""
    context = await get_user_context(PRIMARY_USER_ID)
    
    if context.is_sleeping_hours and threat.priority < Priority.IMMEDIATE:
        await auto_ack(threat, reason="sleeping", mark="[AUTO]")
        return True
    
    if context.is_in_transit and threat.priority <= Priority.HIGH:
        await auto_ack(threat, reason="in_transit", mark="[AUTO]")
        # Все одно повідомляємо, але без вимоги callback
        return True
    
    return False
```

Backup бачить у мирорі:
```
14:23  → Ежик: HIGH Шахед
14:23  [AUTO] Auto-ack reason=sleeping (Ежик спить за routine)
```

### 11. Public-friendly channel (bot/public_status.py)

Створюємо публічний Telegram-канал `@yozhyk_status` де бот постить vague статуси для її друзів і родини. Без точних локацій або деталей загрози.

```python
PUBLIC_STATUSES = {
    "safe":      "🟢 Ежик у безпеці",
    "watching":  "🟡 Ежик у спостереженні (тривога у області)",
    "alert":     "🟠 Ежик у тривозі",
    "shelter":   "🔴 Ежик в укритті",
    "all_clear": "🟢 Відбій. Ежик у безпеці",
}

async def update_public_status(new_status: str):
    if new_status == settings.last_public_status:
        return  # не дублюємо
    
    text = PUBLIC_STATUSES[new_status] + f"\n\nОновлено: {now().strftime('%H:%M')}"
    
    # Edit existing pinned message instead of spamming new ones
    pinned = await db.get_pinned_status_message_id()
    if pinned:
        ...
        msg = await bot.send_message(PUBLIC_CHANNEL_ID, text)
        await bot.pin_chat_message(PUBLIC_CHANNEL_ID, msg.message_id)
        await db.set_pinned_status_message_id(msg.message_id)
    
    settings.last_public_status = new_status
```

Її друзі підписуються на канал → отримують спокій без розкриття приватного.

### 12. Conversation memory + long-term Mishka

Розширення фічі №1 — Мишка пам'ятає попередні розмови довгостроково.

**Архітектура:**
- `sqlite-vec` extension для vector search у тій же SQLite БД
- OpenAI `text-embedding-3-small` для embeddings (дешево, ~$0.02/M tokens)
- Compressed summaries попередніх тижнів зберігаються як окремі "memory points"

```python
async def query_long_term_memory(query: str, top_k: int = 5) -> list[MemoryPoint]:
    """Семантично шукає релевантні минулі діалоги."""
    query_emb = await openai_client.embeddings.create(
        model="text-embedding-3-small",
        input=query,
    )
    results = await db.vec_search(
...
# У conversation manager:
async def respond_with_memory(user_msg: str) -> str:
    relevant_past = await query_long_term_memory(user_msg)
    context = build_context_with_memory(relevant_past)
    # ... feed to Claude Sonnet 4.6 ...
```

**Приклади як це працює:**
```
Ежик (вторник): У меня болит голова, не могу сосредоточиться
Мишка: Маленькая, попей воды и приляг на 20 минут. Это от стресса. — М.

[через тиждень]

Ежик: опять голова болит
Мишка: Опять? Та же история что в прошлый вторник? Тогда тебе помогло
        поспать. Может, всё-таки к врачу запишемся? — М.
```
...
**Premium фічі:**
- 💬 Будь-який текст не-команду → conversational Mishka chat (Claude Sonnet 4.6)
- 🎙 Будь-яке voice message не-команду → Whisper STT → conversational Mishka → ElevenLabs voice .ogg
- 📷 Будь-яке фото → AI photo assessment "чи це загроза?"
- `/crisis [N=360]` — увімкнути crisis mode на N хвилин
- `/crisis_off` — скасувати раніше часу
- `/doc_add <type>` (з вкладеним фото/файлом) — додати документ у vault
- `/docs` — список документів
- `/doc <type>` — отримати документ (encrypted, тільки primary)
- `/replay <event_id>` — анімований threat replay
- `/podcast_now` — згенерувати тижневий podcast зараз
- `/auto_on [N=120]` — увімкнути auto-respond на N хв
- `/auto_off`
- `/wellness` — поточний рівень стресу + рекомендації
- `/breathing` — guided дихальна вправа (4-7-8) як voice
- `/public_status` — поточний public статус у каналі
...
# === ElevenLabs (для voice .ogg, weekly podcast, conversational Мишки) ===
ELEVENLABS_API_KEY=
ELEVENLABS_VOICE_ID=     # ID склонованого голосу Мишки
ELEVENLABS_PLAN=creator   # creator або pro для voice cloning + 30K chars/міс

# === Premium фічі ===
PUBLIC_STATUS_CHANNEL_ID=    # @yozhyk_status або подібне (створи через @BotFather → /newchannel)
DOC_VAULT_AGE_KEY=           # age public ключ для document vault (окремий від bekap-keys)
CRISIS_MODE_DEFAULT_MIN=360  # 6 годин default tривалість

# === Vector memory (sqlite-vec extension) ===
SQLITE_VEC_PATH=/usr/lib/sqlite/vec0.so   # шлях до compiled extension
...
## ОПЕРАЦІЙНИЙ БЮДЖЕТ (verified May 2026, **з усіма premium фічами ~$36–46/міс**)
...
| **Anthropic Haiku 4.5** | vision (~3000 img/міс) + photo assessment + memory compression | ~$6 |
| **Anthropic Sonnet 4.6** | **Conversational Mishka** + crisis-mode reasoning | ~$4 |
| **Gemini 2.5 Flash-Lite** | parallel race з Claude для vision | ~$3 |
| **OpenAI gpt-4o-mini-tts** | TTS для коротких voice .ogg | ~$1 |
| **OpenAI Whisper** | STT для голосових від ежика → conversational chat | ~$1 |
| **OpenAI text-embedding-3-small** | vector memory для long-term Mishka | ~$1 |
| **ElevenLabs Creator** | voice clone Мишки + weekly podcast (3 хв) + voice .ogg | ~$11 |
| Telegram Bot API (primary + emergency + public channel) | — | $0 |
| **Stadia Maps** | ~5K тайлів/міс, free dev plan | $0 |
| **Backblaze B2** | encrypted backups, ~5GB | ~$0.50 |
| **СУМАРНО recurring (з усіма premium)** | | **~$39/міс** |
| **СУМАРНО якщо знімемо ElevenLabs** | OpenAI TTS для всього | **~$28/міс** |
| ~~SMS~~ | DISABLED у v1 | $0 |
| ~~Plivo + Sinch voice calls~~ | DISABLED у v1 (Telegram-only) | $0 |
...
**Burst expenses:**
- **Crisis mode** активний: +$2-3 за добу через підвищений polling та більше LLM calls
- Threat replay GIFs: amortized у Anthropic vision (~$0.20/major event)

**Чи варті premium фічі $13/міс?** Так: conversational Mishka — найбільший emotional value daily. Weekly podcast — ritual moment. Photo assessment — справді життя-зберігаюча. Без них v1 виглядав би "сухо".
...
**Week 4: Core features (предіктивні + контекстні)**
29. Pre-alert from launch detection (включаючи стратегічну авіацію → 4-6 годин попередження)
30. Apartment routing (home_layout)
31. Family circle + /circle, /safe
32. /sos команда
33. YASNO blackout integration
34. Post-event self-report
35. Weekly digest
36. NEPTUN моніторинг через їх Telegram-канал як OSINT (у 2026 public API немає)
37. Custom notification sound generator (TTS + siren mix → .m4a для імпорту в iOS)
38. Weather context (open-meteo) — "при сильному вітрі шахед може відхилитись"
39. Intercept detection enhancement — OSINT patterns про роботу ППО

**Week 5: Premium Tier 1 — Mishka як emotional companion**
40. Conversational Mishka chat (Anthropic Sonnet 4.6 з персонажним promptom)
41. Conversation memory: short-term (20 повідомлень) + long-term compressed summaries
42. Vector memory: sqlite-vec + OpenAI embeddings для semantic recall
43. Trauma-aware messaging (stress metrics → tone adaptation)
44. Threat replay GIF (animated WebP після major event)
45. Voice STT через Whisper для conversational input

**Week 6: Premium Tier 2 — Smart features**
46. AI photo assessment (vision LLM з safety-trained prompt)
47. Crisis mode `/crisis` з підвищеним polling і ескалацією
48. Smart shelter recommender (baseline + ML після 30 днів даних)
49. Weekly podcast (ElevenLabs Мишка-голос, неділя 09:00)
50. Battery prediction model (linear regression на історії заряду)

**Week 7: Premium Tier 3 — Polish + Resilience**
51. Document vault `/docs` (age-encrypted)
52. Auto-respond mode (motion sensor / routine / quiet hours)
53. Public-friendly Telegram channel зі статусами
54. **Telegram Web App** — інтерактивна карта з малюванням зон, історія загроз
55. Mic detection PWA (окремий міні-проект, опц.)
56. Hardening: load tests, chaos engineering, kill-switch tests, monthly fire drill setup
57. v1 production launch і моніторинг 2 тижні
...
### Premium (всі 12 фіч включені у v1)

- [x] Conversational Mishka chat (Claude Sonnet 4.6 з персоною)
- [x] Short-term memory (last 20 messages)
- [x] Long-term memory з compressed summaries (Claude Haiku)
- [x] Vector memory через sqlite-vec + OpenAI embeddings
- [x] Trauma-aware messaging — stress metrics + adaptive tone
- [x] Threat replay GIF — animated WebP після major event
- [x] AI photo assessment (vision LLM з safety prompt)
- [x] Crisis mode `/crisis` — підвищений polling, ескалація на крок вище, ігнор quiet hours
- [x] Smart shelter recommender (baseline + ML після 30 днів даних)
- [x] Weekly podcast (ElevenLabs Мишка-голос, неділя 09:00)
- [x] Document vault `/docs` (age-encrypted сховище)
- [x] Battery prediction (linear regression model)
- [x] Auto-respond mode (motion / routine / quiet hours триггери)
- [x] Public-friendly Telegram channel (vague статуси для друзів)
- [x] Voice STT через Whisper для conversational input
- [x] `/breathing` guided дихальні вправи
...
- [x] sqlite-vec extension для vector memory

---

## ІДЕЇ ДЛЯ v2 (не включаються у v1 — фокус, не distraction)

Voice STT, photo verification та більшість премiум-фіч вже у v1 (вище). У v2 залишаються пункти що потребують серйозного infrastructure-зусилля або зовнішніх ресурсів:
...
3. **Telegram Web App full UI** — повна заміна команд на GUI: налаштування, family circle dashboard, історія з timeline і фільтрами (у v1 Web App лише для малювання зон)
4. **Microphone detection PWA** — мікрофон iPhone детектує характерний шум шахеда (turbojet 4-6kHz або шахед-136 propeller signature)
5. **Custom iOS app** з Critical Alerts entitlement — обхід DND на системному рівні (потребує $99/рік Apple Developer Program)
6. **Weather context integration deeper** — open-meteo для опадів (дощ погіршує курс шахеда, fog reduces vision)
7. **Air defense map** — якщо колись будуть public-friendly дані про позиції ППО
8. **Telegram Stories from bot** — постити поточний threat status як Story
9. **OSINT channel reputation learning** — ML що learns яким каналам більше довіряти на основі історичних збігів з alerts.in.ua
10. **Calendar integration** — попереджати про загрози у місцях запланованих подій
11. **Reverse OSINT** — коли OSINT каже "Парус", бот резолвить у координати і генерує threat cone
12. **Multiple primary users** — моніторити кількох близьких людей одним ботом
13. **AR Kit drone visual recognition** через камеру iPhone — детект візуальний на льоту
14. **Скріншот Google Maps з обведеною областю → vision LLM витягує polygon points** для швидкого зонування
15. **Smart Home integration** — Google Home / Yandex Station голосово оголошує тривогу у квартирі
16. **Federated bots** — якщо її мама/папа теж матимуть AirGuard, кросс-нотифікації між circles
