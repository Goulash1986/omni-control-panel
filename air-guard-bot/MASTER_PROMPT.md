...
**Адмін (тільки backup-користувач):**
- `/latency_stats` — p50/p95/p99 latency за 24г
- `/verify` — перевірити HMAC-підпис повідомлення (anti-spoofing)
- `/test_alert <priority>` — синтетичний alert для перевірки
- `/test_blackout` — синтетичне DTEK сповіщення
- `/fire_drill` — запустити негайно тренувальну сесію
...
**Week 3: Telegram-only ескалація + Resilience**
...
22. Check-in logic + escalation до backup
23. Intercept detection (нові patterns) + state INTERCEPTED
24. Live Location ingestion (📎 → geolocation) + dynamic origin для ETA
25. /set_zone — користувач малює зону моніторингу
